...
plot.groupviolins = function(data, adjust=1.5, n=128, cut=1, 
  ...
  num.pergroup = length(data[[1]])
  names.groups = names(data)
  names.sub = names(data[[1]])

  if (is.null(xlim)) {
    xlim = c(-space/2, (n.groups-1)*space + (num.pergroup*n.groups)+ space/2)
  }
  if (is.null(ylim)) {
  ...
  if (show.xaxis) {
    ...
    group.centers = (i.groups-1)*num.pergroup +
      (i.groups-1)*space + num.pergroup/2

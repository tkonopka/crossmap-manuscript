...
# hp-mp translations
source("hpmp.R")
hpmp.diffusion.text <- c("incisors", "prominent", "axillary", "antitragus")
hpmp.diffusion.ids <- c("MP:0001008", "HP:0011250", "HP:0006957",
                        "HP:0006844", "HP:0002330")
...
```{r fig.calibration, eval=TRUE, echo=FALSE, fig.width=6.4, fig.height=4.2, fig.cap="\\textbf{Diffusion processes for human-to-mouse phenotype translation.} \\textbf{(A)} Top features imputed by a diffusion process starting from a query with text ‘incisors’. Diffusion transfers weight from k-mers present in the query to other k-mers associated with teeth. K-mers within the query retain large weights, but are omited from the figure to emphasize the ranking of the newly-imputed features. \\textbf{(B)} Calibration of the strength of diffusion for HP-MP translation. Queries from the HP ontology were diffused with different strengths, and then mapped to MP terms using the search algorithm. Precision was measured against mappings produced by owlsim. The shaded area of the graph is reproduced with a zoomed scale on top. \\textbf{(C)} Analogous to (B), but measuring the effect of diffusion in terms of path length. \\textbf{(D)} Top features imputed for an entire HP query composed of a phenotype title (‘Absent patellar reflexes’), description, synonyms, and other text data. Bars show the weights assigned by a diffusion process driven by the MP ontology, and by a process also including manual annotations. \\textbf{(E)} Calibration of strengths of diffusion in terms of precision, using diffusion driven by the MP ontology and manual annotations. \\textbf{(F)} Analogous to (E), but measuring the impact of diffusion in terms of path length."}
layout(matrix(c(1,2,4,
                1,3,5,
                6,7,8), ncol=3, nrow=3, byrow=T),
		widths=c(1.6, 2.4, 2.4), heights=c(0.6, 1.5, 2.1))
#
...
calibration.pathlen.zoom <- c(1.34, 1.52)
plot.calibration.lines(calibration.targets,x="diff_targets",
...
plot.calibration.lines(calibration.targets,x="diff_targets",
                       ...
                       Rcssclass="pathlen")
#
# comparison of two diffusion profiles
example.hp <- get.diffusion.comparison(hpmp.diffusion.examples, diffused.best.configs,
                                      pair.label=c("auto", "manual"))
example.hp <- example.hp[input=="HP:0006844"]
plot.diffusion.barplot(example.hp,
                       value.cols=c("value_auto", "value_manual"),
                       n=18, xlim=c(0, 0.35), 
                       legend.pos=c(0.14, -13.5),
